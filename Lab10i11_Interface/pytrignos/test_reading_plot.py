...
class Scope:
    def __init__(self, ax, maxt=4, dt=0.02):
        ...
        self.ax.set_ylim(-.001, 0.001)
        self.ax.set_xlim(0, self.maxt)

    def update(self, val, sensors):
        saved: pd.DataFrame = np.zeros(100)
        ...
        if self.ydata==None:
            saved = np.append(saved, self.ydata)
            print(saved)
        return self.line,
...
trigno_sensors.add_sensors(sensors_mode='EMG', sensors_ids=(6,), sensors_labels=('EMG1',), host='150.254.46.37')
